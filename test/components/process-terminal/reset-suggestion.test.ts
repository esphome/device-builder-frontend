...
describe("shared reset-suggestion renderers", () => {
  ...
  it("remote reset link is disabled while a reset is in flight", () => {
    const host = makeHost();
    host._remoteResetPending = true;
    const tree = renderBuildFailureSuggestion(host, "Receiver A", "a".repeat(64));
    const matches = findTemplatesByAnchor(tree, 'class="reset-suggestion"');
    expect(matches[0].values).toContain(true); // the ?disabled binding
  });
});
