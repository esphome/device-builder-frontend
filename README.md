...
- In-browser flashing and logs code lives in one directory per platform under `src/platforms/` (`esp/`, `nrf52/`, `rp2/`, `rtl87xx/`), with tests mirrored under `test/platforms/`. Each has an `index.ts` for the API both the Device Builder and ESPHome Web use, and a `dashboard.ts` for the Device Builder's install flow; `src/` code outside a platform imports only those two. A flash engine loaded on demand is exported from `index.ts` only as types plus a `load*` function, so it stays out of the main chunk. The one exception is the LibreTiny UF2 parser in the Device Builder, whose RTL8720C flow imports it directly; web.esphome.io loads it on demand. ESPHome Web's own cards and install dialogs for each platform live in `src/web/platforms/<name>/` (imported by file; the entry-file rule is for `src/platforms/`), with tests under `test/web/platforms/`.
- A platform with an in-app compile-then-flash flow in the Device Builder (nRF52 DFU, Pico UF2, RTL8720C ROM) exports one `BrowserFlasher` descriptor from its `dashboard.ts` (see `src/components/firmware-install-dialog/browser-flasher.ts`) and gets one entry in `src/platforms/browser-flashers.ts`, the one module outside a platform that imports every `dashboard.ts`. The install dialog, the install-method rows and `applyInstallMethod` read the platform's steps, copy, buttons and Retry target from it, so a new platform needs no edits there. ESP's flows are still built into the dialog.
