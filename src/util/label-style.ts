 ...
 *
 * Labels carry an optional ``#rrggbb`` color picked by the user.
 * We render the chip on a tinted background derived from that color
 * with a contrasting text colour so the chip stays readable on both
 * light and dark themes. Labels without an explicit color fall back
 * to a neutral surface palette so the chip still reads as "a chip"
 * (and not as accidentally-themed default text).
 */
...
/** Pick the contrasting foreground for a hex color using the
 *  WCAG-style relative-luminance heuristic. The cutoff is biased
 *  toward darker text on mid-tone colors (matching GitHub's label
...
function contrastingTextColor(hex: string): string {
  ...
  // Use the simpler relative-luminance approximation; the chip
  // surface is dimmed by a transparent overlay, so the strict WCAG
  // formula isn't necessary.
