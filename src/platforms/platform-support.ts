 ...
 * RTL8720C): its in-app compile-then-flash flow. Each platform's
 * ``dashboard.ts`` exports one ``PlatformSupport``, ``registry.ts`` lists
 * them, and the install dialog, the method rows and ``applyInstallMethod``
 * read everything platform-specific from it. ESP is still built into those.
 *
 * Adding a platform:
