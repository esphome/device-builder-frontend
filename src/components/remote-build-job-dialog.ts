 ...
 * events through ``buildOffloadJobsContext``.
 *
 * Cancellation while running isn't supported here — the
 * ``cancel_job`` reverse-direction control message is
 * deferred to phase 5d. Closing the dialog mid-build leaves
 * the job running on the receiver; the user can re-open to
 * see live progress until terminal.
 *
 * Dispatches ``remote-build-job-submitted`` with the job
 * seed (pin / receiver_label / configuration / target /
 * job_id) on a successful ack so the parent app-shell can
 * stamp the in-flight jobs map's display fields. The wire
 * frames don't carry those fields; without the dispatch the
...
export class ESPHomeRemoteBuildJobDialog extends LitElement {
   ...
   *  target's pin + display label; everything else is
  ...
  static styles = [
    ...
    css`
      ...
      .field-error {
        color: var(--esphome-danger, #c0392b);
      ...
      .status-running {
        background: color-mix(
          in srgb,
          var(--esphome-info, #2563eb),
          transparent 80%
        );
        color: var(--esphome-info, #2563eb);
      }
