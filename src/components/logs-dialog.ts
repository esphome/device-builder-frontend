...
export class ESPHomeLogsDialog extends LitElement {
  ...
  // The RTS-pulse Reset Device works here (an RTL8720C kit resets through
  // CEN on RTS, with DTR kept low so the strap stays released). A Pico or an
  // nRF52 has no reset line on its CDC and the pulse's DTR drop only detaches
  // the host; a Pico resets through the session's hook instead (WebUSB
  // browsers).
  _pulseResets = true;
